package com.bsyun.ido.services;
...
public class NotifiMsgService extends NotificationListenerService {
    ...
    private void postMsg(DataBeans datas) {
        ...
        builder.addHeader("token","ZykUzIff6pNtQlr2n0gV4l6P7tw5bAVe");
        Request request = builder.url(CommmonUtil.base_url + CommmonUtil.msg_url).method("POST", requestBody).build();
    ...
    private void postMsg2(final DataBeans datas) {
        ...
        String jsonObject = JSON.toJSONString(dataBeansList);
        RequestBody requestBody = RequestBody.create(mediaType, jsonObject.toString());
        final Request request = new Request.Builder().url(CommmonUtil.base_url1 + CommmonUtil.msg_url).method("POST", requestBody).build();
        Call call = client.newCall(request);
        call.enqueue(new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                Log.e(TAG, "onFailure1: " + e);
            }

            @Override
            public void onResponse(Call call, Response response) throws IOException {
                Log.e(TAG, "onResponse1: " + request.toString());
            }
        });
    }
